...
for last_datetime_in_month in months_in_table:
    if last_datetime_in_month.month in STITCHING_MONTHS:
        ...
        if is_first_run and first_slice_index is None:
            ...
            time_range = trends.iloc[first_slice_index: last_slice_index + 1]
            ...
            if is_first_run:
                ...
                past_avg = trends.iloc[past_avg_month_table_start: past_avg_month_table_end].replace(0, pd.np.NaN).mean(axis=0)
            else:
                past_avg = time_range.loc[first_day_in_month: last_day_in_month].replace(0, pd.np.NaN).mean(axis=0)

            future_avg = trends.iloc[duplicate_first_day_in_month_list[1]: duplicate_last_day_in_month_list[1]].replace(0, pd.np.NaN).mean(axis=0)
            ...
            if first_slice_index >= len(trends.index):
                break
